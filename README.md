...
The current build is a complete local vertical slice: choose a home borough, trade twelve products over thirty days, learn durable interborough spreads, develop uncertain local contacts, use the bank and loan shark at home, and build a volume business across rented storage. Heat, accumulated exposure, violence, debt, health, guns, and coat capacity shape the risk. Unsold stock is worthless at Day 30. Runs save automatically in the browser; the runner name and completed scores remain on that device.
...
- `src/test/` covers replay, accounting, split-proof heat, passive cooling, hidden market structure, contacts and forecasts, financing, unified enforcer collection, local and remote storage trades, fence discounts, rent arrears, police volleys, deliberately unbalanced strategies, Day 30 settlement, multi-seed invariants, save migration, and personal-score retention.
...
- Balance tuning and event volume remain provisional; the complete rules contract and current constants are recorded in `DESIGN-NOTES.md`.
- The initial Jet-event set is deliberately small.
- The responsive one-column layout is functional across narrow and wide screens, but still benefits from hands-on device review.
